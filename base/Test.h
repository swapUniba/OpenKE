...
void test_link_prediction() {
	...
	//@luigiba EDIT
	//changed output stream from console to file
	//START EDIT
	FILE* fout = fopen((inPath + "test_LINK_PREDICTION_RESULTS.txt").c_str(), "w");																		
	
    fprintf(fout, "no type constraint results:\n");
    
    fprintf(fout, "metric:\t\t\t MRR \t\t MR \t\t hit@10 \t hit@3  \t hit@1 \n");
    fprintf(fout, "l(raw):\t\t\t %f \t %f \t %f \t %f \t %f \n", l_reci_rank, l_rank, l_tot, l3_tot, l1_tot);
    fprintf(fout, "r(raw):\t\t\t %f \t %f \t %f \t %f \t %f \n", r_reci_rank, r_rank, r_tot, r3_tot, r1_tot);
    fprintf(fout, "averaged(raw):\t\t %f \t %f \t %f \t %f \t %f \n",
            (l_reci_rank+r_reci_rank)/2, (l_rank+r_rank)/2, (l_tot+r_tot)/2, (l3_tot+r3_tot)/2, (l1_tot+r1_tot)/2);
    fprintf(fout, "\n");
    fprintf(fout, "l(filter):\t\t %f \t %f \t %f \t %f \t %f \n", l_filter_reci_rank, l_filter_rank, l_filter_tot, l3_filter_tot, l1_filter_tot);
    fprintf(fout, "r(filter):\t\t %f \t %f \t %f \t %f \t %f \n", r_filter_reci_rank, r_filter_rank, r_filter_tot, r3_filter_tot, r1_filter_tot);
    fprintf(fout, "averaged(filter):\t %f \t %f \t %f \t %f \t %f \n",
	...
	//changed output stream from console to file
	//START EDIT
    fprintf(fout, "type constraint results:\n");
...
void test_triple_classification(REAL *relThresh, REAL *score_pos, REAL *score_neg) {
	...
	//@luigiba EDIT
	//created new file with classificator output for each test triple
	FILE* fout = fopen((inPath + "test_CLASSIFICATION.txt").c_str(), "w");			
	...
	//@luigiba EDIT
	FILE* fout1 = fopen((inPath + "test_CLASSIFICATION_RESULTS.txt").c_str(), "w");	
    fprintf(fout1, "triple classification accuracy is %lf\n", aveAcc);				
	fclose(fout1);																	
}
